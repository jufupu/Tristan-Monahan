...
export class ExpenseEntryService {
  ...
  addExpenseEntry(expenseEntry: ExpenseEntry): Observable<ExpenseEntry> {
    return this.httpClient.post<ExpenseEntry>(this.expenseRestUrl, expenseEntry, this.httpOptions).pipe(retry(3), catchError(this.httpErrorHandler));
  }
 ...
 deleteExpenseEntry(expenseEntry: ExpenseEntry | number) : Observable<ExpenseEntry> {
  ...
  return this.httpClient.delete<ExpenseEntry>(url, this.httpOptions).pipe(retry(3), catchError(this.httpErrorHandler));
 }

  private httpErrorHandler (error: HttpErrorResponse){
    if(error.error instanceof ErrorEvent){
      console.error("A client side error has occurred. The error message is " + error.message);
    } else{
      console.error("An error occurred on the server. Http status code: " + error.status + "Http status: " + error.message);
    }
